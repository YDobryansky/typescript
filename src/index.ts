...
const menu: Pizza[] = [
  { id: nextPizzaId++, name: 'Margherita', price: 18, address: { street: 'LA' } },
...
const addNewPizza = (pizzaObj: Omit<Pizza, 'id'>): Pizza => {
  ...
  const newPizza: Pizza = { ...pizzaObj, id: nextPizzaId++ };
  menu.push(newPizza);
  return newPizza;
};
...
const placeOrder = (pizzaName: string): Order | void => {
...
const addToOrders = <T,>(array: T[], item: T): T[] => {
  array.push(item);
  return array;
};

addToOrders<Pizza>(menu, { id: nextPizzaId++, name: 'Chilly', price: 20 });
addToOrders<Order>(orderQueue, { id: nextOrderId++, pizza: menu[2], status: 'completed' });
